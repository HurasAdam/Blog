...
const ManagePosts: React.FC = () => {
    ...
    const submitSearchKeywordHandler = (e: React.ChangeEvent<HTMLInputElement>): void => {
        e.preventDefault();
        setCurrentPage(1);
        refetch()
    }
                                        ...
                                        posts?.data.map((post) => {
                                            return (
                                                <tr>
                                                        ...
                                                        <div className="flex items-center">
                                                            <div className="flex-shrink-0">
                                                                <a href="/" className="relative block">
                                                                    <img alt={post.title} src={post?.photo ? post?.photo : images.Post3} className="mx-auto object-cover rounded-lg h- w-10 aspect-square " />
                                                                </a>
                                                            </div>
                                                            <div className="ml-3">
                                                    ...
                                                    <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                                                        <p className="text-gray-900 whitespace-no-wrap">
                                                            {post?.categories.lenght > 0 ? post.categories[0] : "Uncategorized"}
                                                        </p>
                                                    </td>
                                                    <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                                                        <p className="text-gray-900 whitespace-no-wrap">
                                                    ...
                                                    <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
                                                        <div className="flex gap-x-1.5 gap-y-1.5 flex-wrap">
                                                            {post?.tags.length > 0 ? post?.tags.map((tag, index) => {
                                                                ...
                                                                return (
                                                                    <span className="bg-blue-400 py-1 px-1 rounded-[5px] font-bold text-white " key={index}>
